...
void initEnemies()
{
	for (int i = 0; i < MAX_ENEMIES; i++)
	{
		...
		if (found >= 0)
		{
			...
			enemy[i]->speed = 1; //bug: weiapp0:rd bug makes the enemies spawn out side of the viewport if speed is initialized here
			enemy[i]->goLeft = 0;
...
void updateEnemies()
{
	...
	for (int e = 0; e < MAX_ENEMIES; e++) if (enemy[e])
	{
		if (enemy[e]->goLeft == 0)
		{
			...
			enemy[e]->speed = 150; //bug: an weiapp0:rd bug makes the enemies spawn out side of the viewport if speed is initialized before this point
		}
...
int main(int argc, char* argv[])
{
	...
	//load audio
	snd_blaster = Mix_LoadWAV("app0:rd/blaster.ogg");
	snd_explo = Mix_LoadWAV("app0:rd/explode1.ogg");
	snd_pusher = Mix_LoadWAV("app0:rd/pusher.ogg");
	music = Mix_LoadMUS("app0:rd/bodenstaendig.ogg");
	...
	//init font stuff
	vermin_ttf = TTF_OpenFont("app0:rd/vermin_vibes_1989.ttf", 24);
	char textBuffer[64];

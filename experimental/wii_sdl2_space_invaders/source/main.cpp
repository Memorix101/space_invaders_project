...
void addExplo(float x, float y)
{
	...
	if (found >= 0)
	{
		...
		//printf("explo_surface->w: %d\n", explo_surface->w);
		//printf("explo_surface->h: %d\n", explo_surface->h);
		//printf("explo_x: %d\n", explo[i]->pos.x);
		//printf("explo_y: %d\n", explo[i]->pos.y);
	}
}
...
void updateExplo()
{
	...
	for (int i = 0; i < MAX_EXPLO; i++)
	{
		if (explo[i])
		{
			explo[i]->vec = {explo[i]->rect.x, 0, 64, 64};
...
void drawExplo()
{
	for (int i = 0; i < MAX_EXPLO; i++)
	{
		if (explo[i])
		{
			/*printf("Draw boom at (%d, %d) with frame (%d, %d, %d, %d)\n",
				   explo[i]->pos.x, explo[i]->pos.y,
				   explo[i]->vec.x, explo[i]->vec.y, explo[i]->vec.w, explo[i]->vec.h);*/
			SDL_Rect drect = {0, 0, 64, 64};
			SDL_Rect rect = {explo[i]->pos.x, explo[i]->pos.y, explo_surface->w, explo_surface->h};
			SDL_RenderCopy(renderer, explo[i]->tex, &explo[i]->vec, &explo[i]->spritecut);
		}
	}
}
...
int main(int argc, char *argv[])
{
	...
	window = SDL_CreateWindow(nullptr, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
							  640, 480, SDL_WINDOW_FULLSCREEN); // SDL_WINDOW_FULLSCREEN
	renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_ACCELERATED);
	SDL_RenderSetLogicalSize(renderer, 640, 480);

	SDL_ShowCursor(SDL_DISABLE); // hide wii pointer
	...
	if (SDL_NumJoysticks() > 0)
	{
		for (int i = 0; i < SDL_NumJoysticks(); i++)
		{
			...
			{
				printf("Joystick %d is not a GameController-compatible device\n", i);
			}
		}
	}

	load_assets();
	music = Mix_LoadMUS("sd:/space-invaders/rd/bodenstaendig.mp3");
	...
	vermin_ttf = TTF_OpenFont("sd:/space-invaders/rd/vermin_vibes_1989.ttf", 24);
	char textBuffer[64];
	...
	// load audio
	snd_blaster = Mix_LoadWAV("sd:/space-invaders/rd/blaster.mp3");
	snd_explo = Mix_LoadWAV("sd:/space-invaders/rd/explode1.wav");
	snd_pusher = Mix_LoadWAV("sd:/space-invaders/rd/pusher.wav");
	...
	while (quit == 0)
	{
		...
		while (SDL_PollEvent(&e) != 0)
		{
			...
			case SDL_CONTROLLERBUTTONDOWN:
				...
				if ((e.cbutton.button == SDL_CONTROLLER_BUTTON_A && player.alive) ||
					(e.cbutton.button == SDL_CONTROLLER_BUTTON_B && player.alive) ||
					(e.cbutton.button == SDL_CONTROLLER_BUTTON_X && player.alive) ||
					(e.cbutton.button == SDL_CONTROLLER_BUTTON_Y && player.alive) ||
					(e.cbutton.button == SDL_CONTROLLER_BUTTON_RIGHTSHOULDER && player.alive))
				{
					addBullet(player.pos.x + player.size.w / 2 - 3, player.size.y);
					Mix_PlayChannel(-1, snd_blaster, 0);
				}

				if (e.cbutton.button == SDL_CONTROLLER_BUTTON_START && gameover == 1)
				{
					reset();
				}

...
BITMAP *gameover_ui_tex = NULL;
BITMAP *win_ui_tex = NULL;
...
void preload_asset()
{
  ...
  explo_tex = load_tga("/rd/explode.tga", NULL);
  gameover_ui_tex = load_tga("/rd/gameover_ui.tga", NULL);
  win_ui_tex = load_tga("/rd/win_ui.tga", NULL);
  //fontTex = load_bitmap("/rd/font.bmp", NULL);
  blaster_snd = load_sample("/rd/blaster.wav");
  pusher_snd = load_sample("/rd/pusher.wav");
  explode_snd = load_sample("/rd/explode1.wav");
  //music = load_midi("/rd/utr.mid");
  //music = load_sample("/rd/music.wav");
}
...
void updateEnemies()
{
  ...
  for (e = 0; e < MAX_ENEMIES; e++) if (enemy[e]) {
    ...
    if (enemy[e]->shoot == 1 && enemy[e]->alive == 1)
    {
      ...
      play_sample(pusher_snd, 240, 0, 1000, 0);
    }
...
*/
    ...
    {
      //printf("SHOOT! \r\n");
      addBullet(player.pos.x + (player.hitbox.w / 2) - 2, player.pos.y - 7);
      play_sample(blaster_snd, 240, 0, 1000, 0);
      A_BUTTON_DOWN = 1;
    }
  ...
  for (i = 0; i < MAX_BULLETS; i++) if (bullets[i])
  {
    for (e = 0; e < MAX_ENEMIES; e++) if (enemy[e]) {
        ...
        enemy[e]->alive == 1)
      {
        ...
        removeBullet(i);
        play_sample(explode_snd, 240, 0, 1000, 0);
        score += 100;
        enemies_killed--;
        //printf("BOOM!\n");
        break;
...
void reset()
{
	...
	itemCount = 0;
	initEnemies();
	p_move = 320 / 2 - player.hitbox.w / 2; //player.pos.x = 640 / 2 - player.size.w / 2;
	player.alive = 1;
}
...
if (install_sound(DIGI_AUTODETECT, MIDI_AUTODETECT, NULL) != 0) {
  ...
  //play_sample(music, 240, 0, 1000, -1);
  ...
  while (!key[KEY_ESC])
  {
    ...
    if (start_game == 1)
    {
      ...
      else
      {
        ...
        textout_centre_ex(buffer, font, "PRESS ENTER TO RESTART", SCREEN_W / 2, SCREEN_H / 2 + 24, makecol(255,255,255), -1);
        //masked_blit(gameover_ui_tex, buffer, 0, 0, 0, 0, std_res.w, std_res.h);
        gameover = 1;
      }
	    ...
	    if(enemies_killed <= 0)
      {
        ...
        textout_centre_ex(buffer, font, "PRESS ENTER TO RESTART", SCREEN_W / 2, SCREEN_H / 2 + 24, makecol(255,255,255), -1);
        gameover = 1;
        //masked_blit(win_ui_tex, buffer, 0, 0, 0, 0, std_res.w, std_res.h);
      }		    
    }
